...
signac$high.nuclesomeSignal <- ifelse(signac$nucleosome_signal >5, 'High', 'Low')
FragmentHistogram(signac, group.by = 'high.nuclesomeSignal') + NoLegend()

table(signac$MULTI_ID,signac$high.nuclesomeSignal)

VlnPlot(signac,
            features = c("on_target_fragments",
                         "nFeature_bin5000",
                         "nCount_HTO",
                         "passed_filters",
                        #"mitochondrial",
                         "EnhPromDNAseHyp_ratio",
                         "nCount_RNA",
                         "nFeature_RNA",
                      #   "HTO_margin",
...
DimPlot(signac,group.by="MULTI_ID", reduction = 'lsi_umap', label.box = T, label=T)+NoLegend()
...
VlnPlot(signac, "passed_filters", log=T)
FeaturePlot(signac, "nucleosome_signal", max.cutoff = 3, min.cutoff = 2)
```
...
p_markers<-FeaturePlot(
  object = signac,
  features = c(markers, "nFeature_RNA"),
  pt.size = 0.1,
  max.cutoff = 'q90',
