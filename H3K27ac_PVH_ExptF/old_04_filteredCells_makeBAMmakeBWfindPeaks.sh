...
mkdir seacr -p
mkdir seacr/bed -p
mkdir seacr/tmpBAM -p


## For all cellranger fragments:
for i in ${projPath}/bam/${ExptName}_AllCellrangerFragments.bam; do 
pathName=${i%.bam};clusterName=${pathName#$projPath/bam/}
#samtools
samtools view -b -f 2 -F 1804 $i > ${projPath}/seacr/tmpBAM/${clusterName}_nodup.bam                               
...
#bedtools
#bedtools bamtobed -bedpe -i ${projPath}/seacr/tmpBAM/${clusterName}.sorted.bam > ${projPath}/seacr/bed/${clusterName}.bed
#bedtools intersect -v -a ${projPath}/seacr/bed/${clusterName}.bed -b $blacklist_mm10 > ${projPath}/seacr/bed/${clusterName}.white.bed
#awk '$1==$4 && $6-$2 < 1000 {print $0}' ${projPath}/seacr/bed/${clusterName}.white.bed > ${projPath}/seacr/bed/${clusterName}.clean.bed
#cut -f 1,2,6 ${projPath}/seacr/bed/${clusterName}.clean.bed | sort -k1,1 -k2,2n -k3,3n > ${projPath}/seacr/bed/${clusterName}.fragments.bed
#bedtools genomecov -bg -i ${projPath}/seacr/bed/${clusterName}.fragments.bed -g ${genome} > ${projPath}/seacr/bed/${clusterName}.fragments.bedgraph
