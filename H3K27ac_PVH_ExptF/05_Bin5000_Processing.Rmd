...
```{r, cache=F}
VlnPlot(signac, c("nucleosome_signal","passed_filters", "nFeature_bin5000"), log=T)

signac<-
  subset(signac,
           nucleosome_signal < 4 &
	nucleosome_signal > 1 &
          passed_filters > (10^cutoff_reads_min)*2 &  
     enhancer_region_ratio > cutoff_enhancer_ratio &
     passed_filters < 10^cutoff_reads_max
)

VlnPlot(signac, c("nucleosome_signal", "enhancer_region_ratio"), log=F)
VlnPlot(signac, c("passed_filters", "nFeature_bin5000"), log=T)
...
signac <- FindTopFeatures(signac, min.cutoff = "q25",verbose = T)
...
signac <- RunUMAP(signac, reduction = 'lsi_', dims =c(2:ndim), reduction.name = 'lsi_umap', n.neighbors = 15)
...
signac <- FindClusters(object = signac, verbose = FALSE, algorithm = 3, resolution=BinRes)
...
p_dimplotSamples<-DimPlot(signac, reduction = 'lsi_umap', group.by="HTO_maxID", label =F, label.box = F, repel = T) +ggtitle("Samples based on LSI bin5000")
p_dimplotSamples
```
...
VlnPlot(signac, features=c("passed_filters", "nFeature_bin5000"))  
