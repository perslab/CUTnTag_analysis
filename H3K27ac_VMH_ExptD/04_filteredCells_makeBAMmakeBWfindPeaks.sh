...
samtools index $projPath/bam/${ExptName}_FilteredCells.bam ; cd ..
...
####
## For all cellranger fragments: & For all filtered cells.
for i in ${projPath}/bam/${ExptName}_AllCellrangerFragments.bam $projPath/bam/${ExptName}_FilteredCells.bam;
do pathName=${i%.bam};
clusterName=${pathName#$projPath/bam/}
#######################
mkdir $projPath/macs2/$clusterName/narrow -p
mkdir $projPath/macs2/$clusterName/broad -p
# narrow
macs2 callpeak -t ${i} -g mm -f BAMPE -n $clusterName --outdir ${projPath}/macs2/$clusterName/narrow -q 0.01 -B --SPMR --keep-dup=1 2>&1
# broad
macs2 callpeak -t ${i} -g mm -f BAMPE -n $clusterName --outdir ${projPath}/macs2/$clusterName/broad -q 0.01 -B --SPMR --keep-dup=1 --broad-cutoff=0.1 --broad 2>&1
done
...
for i in ${projPath}/bam/${ExptName}_AllCellrangerFragments.bam $projPath/bam/${ExptName}_FilteredCells.bam; do 
pathName=${i%.bam};
clusterName=${pathName#$projPath/bam/}
